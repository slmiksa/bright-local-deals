...
import { ArrowRight, Phone, MapPin, Clock, Star, Share2 } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-background pb-8 max-w-[430px] mx-auto">
      ...
      <div className="relative aspect-[16/10] overflow-hidden">
        <img src={ad.image} alt={ad.shopName} className="w-full h-full object-cover" />
        {ad.featured && (
          <span className="absolute top-3 right-3 bg-gold text-primary-foreground text-[11px] font-bold px-3 py-1 rounded-xl shadow-elevated flex items-center gap-1">
      ...
      <div className="px-5 pt-5">
        ...
        <div className="mt-5 rounded-2xl overflow-hidden shadow-card bg-card">
          ...
          <a
            href={`https://www.google.com/maps/dir/?api=1&destination=${ad.lat},${ad.lng}`}
            target="_blank"
            rel="noopener noreferrer"
            className="block text-center text-primary font-semibold text-[13px] py-3 active:bg-secondary transition-colors"
          >
            فتح في خرائط قوقل ↗
          </a>
        </div>
        ...
        <div className="mt-5 flex gap-3">
          <a
            href={`tel:${ad.phone}`}
            className="touch-target flex-1 flex items-center justify-center gap-2 bg-primary text-primary-foreground rounded-2xl py-3.5 font-bold text-[14px] active:scale-[0.97] transition-transform shadow-elevated"
          >
            <Phone className="w-5 h-5" />
            اتصل الآن
          </a>
          <a
            href={`https://wa.me/966${ad.phone.slice(1)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="touch-target w-14 h-14 flex items-center justify-center bg-[hsl(142_70%_45%)] text-primary-foreground rounded-2xl active:scale-[0.97] transition-transform shadow-card"
          >
